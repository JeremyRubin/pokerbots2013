...
class AggroModifiers(object):
        ...
        def generate_exponential_strategies(self,currentStats):
            ...
            raiseFreq = 0.60-0.15*math.exp(aggro_level*-10.0)
            checkFreq = 1-raiseFreq
            callFreq = raiseFreq
            raiseLevel = 4.0-3.3*math.exp(aggro_level*-10.0)
            ...
            freq = -6.25
            pflop = 0.50-0.15*math.exp(loose_level*freq)
            posflop = 0.45-0.15*math.exp(loose_level*freq)
            turn = 0.40-0.15*math.exp(loose_level*freq)
            river = 0.35-0.15*math.exp(loose_level*freq)
            '''
            self.LooseMod = {
            'keep_percent_preflop': loose_level,
            'keep_percent_flop': loose_level,
            'keep_percent_turn': loose_level,
            'keep_percent_river': loose_level}
            '''
